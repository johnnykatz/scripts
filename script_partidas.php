...
echo "Procesando...";
if (($fichero = fopen("partidas.csv", "r")) !== FALSE) {
    ...
    while (($datos = fgetcsv($fichero, 1000, ";")) !== FALSE) {
        ...
        if ($i > 0) {
            ...
            //$localidad = isset($arrayDomicilio[1]) && $arrayDomicilio[1] != "" ? $arrayLocalidad[trim($arrayDomicilio[1])] : 1;



            $arrayCalle = explode(',', trim($arrayDomicilio[0]));
            $arrayNumero = explode('N°', trim($arrayCalle[0]));
            if (!isset($arrayNumero[1])) {
                $arrayCalleNumero = preg_split('/([A-Z|Ñ ])(?=\d)/', trim($arrayNumero[0]));
            }
            $calle = $arrayCalleNumero[0];
            $numero = isset($arrayCalleNumero[1]) ? $arrayCalleNumero[1] : "";


            //elimino el guion final y posadas
//            $calle = preg_replace("(-.*POSADAS|.,.*)", "", $calle);
